...
if (-not $ReleaseTitle) {
    ...
    $ReleaseTitle = "quicker-voice-runtime $RuntimeTag"
...
Windows voice **runtime** for QuickerAgent (quicker-voice-v1).
...
Publish-GitHubReleaseAssets `
    ...
    -Assets @($RuntimeZip, $ManifestPath) `
...
SenseVoice model pack for QuickerAgent voice plugin (versionless; update only when model files change).
...
"@
    ...
    Publish-GitHubReleaseAssets `
        ...
        -Title 'SenseVoice model (voice-asr)' `
